...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{../outputs/figures/sample_time_series.png}
\caption{Sample hourly energy consumption time series for three representative households over a one-week period, showing diurnal patterns, weekend effects, and inter-household variability. Each line represents a different household, with consumption normalized to highlight temporal patterns.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.8\textwidth]{../outputs/figures/computational_speedup.png}
\caption{Computational speedup of C++ implementation versus Python baseline for PCA and K-means operations across different dataset sizes (200 to 100,000 households). Error bars indicate standard deviation over 10 runs. The C++ engine maintains consistent 8-12× speedup for large-scale problems, demonstrating scalability for smart meter deployments.}
\label{fig:speedup}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{../outputs/figures/pca_scree_plot.png}
\caption{PCA scree plot showing explained variance by component. The first six components (marked with vertical line) explain 95.6\% of total variance. The sharp drop after the first component indicates a dominant mode of variation, while the elbow around component 6 justifies dimensionality reduction.}
\label{fig:pca_scree}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{../outputs/figures/cluster_validation_metrics.png}
\caption{Clustering validation metrics for K ranging from 2 to 6. (a) Silhouette score peaks at K=3. (b) Calinski-Harabasz index shows diminishing returns after K=3. (c) Davies-Bouldin index is minimized at K=3. The convergence of all three metrics supports K=3 as the optimal number of clusters.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{../outputs/figures/cluster_scatter_pc1_pc2.png}
\caption{Scatter plot of households in the first two principal component space (explaining 76.1\% of variance), colored by cluster assignment. Clusters show moderate separation with some overlap, consistent with the silhouette score of 0.312. Cluster centroids are marked with large symbols.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{../outputs/figures/cluster_feature_heatmap.png}
\caption{Feature comparison heatmap across the three clusters. Each row represents a standardized feature value (z-score), with red indicating above-average and blue below-average values. Clusters differ primarily in consumption magnitude (mean, peak, total) while exhibiting similar patterns in variability metrics (CV, PAR) and temporal distributions.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{../outputs/figures/time_of_use_profiles.png}
\caption{Average hourly consumption profiles for each cluster over a 24-hour period. All clusters exhibit similar diurnal patterns with afternoon peaks (12-17h), evening secondary peaks (18-21h), and nighttime troughs (0-6h), differing primarily in magnitude. Shaded regions indicate ±1 standard deviation within each cluster.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{../outputs/figures/silhouette_analysis.png}
\caption{Silhouette analysis for K=3 clustering solution. Each bar represents a household, colored by cluster assignment and sorted by silhouette coefficient within each cluster. The vertical dashed line marks the overall average silhouette score (0.312). Most households have positive coefficients, indicating appropriate cluster assignment, though some near-boundary cases show lower values.}
