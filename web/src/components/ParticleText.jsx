 ...
 * a cursor-repel field and an optional highlight bloom. Font loading is awaited
 * before sampling, the canvas is device-pixel-ratio aware, and reduced-motion
 * support collapses the animation to a static layout.
 *
 * This file keeps the project's `.jsx` convention (no `.tsx`), so the module
 * resolves as `./components/ParticleText` without ambiguity. Its styles live in
 * the project's single `styles.css` (`.particle-text`, `.__canvas`, `.__sr`).
 */
...
/* ---- helpers (top-level so use of `clamp`/`easeOutCubic` inside the effect
        resolves against these declarations rather than being hoisted) ---- */
